...
export default function KnowledgePage() {
  ...
  const searchMut = useMutation({
    mutationFn: (q: string) =>
      api<SearchResponse>("/api/knowledge/search", {
        method: "POST",
        body: { query: q, scope: "all", topK: 10 },
      }),
  });

  // Live indexing progress: subscribe to per-doc SSE while any doc is indexing
  useEffect(() => {
    const indexing = (list.data?.documents ?? []).filter(
      (d) =>
        d.indexStatus === "pending" || d.indexStatus === "indexing",
    );
    if (indexing.length === 0) return;
    const sources = indexing.map((d) => {
      const url = `/api/stream/${encodeURIComponent(`knowledge.indexing.${d.id}`)}`;
