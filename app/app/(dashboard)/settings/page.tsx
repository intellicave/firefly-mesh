...
//   Account name/title/avatar:  PUT /api/me  →  PATCH /api/employees/:id
//   Org display name:           PUT /api/org →  PATCH /api/organizations/me
//   Org slug edit:              not exposed by hub PATCH; field is read-only
//                               in sprint A (slug change recovery in sprint B)
...
import { fetchMe, type MeEmployee, type MeResponse } from "@/lib/me";
import {
  Avatar,
  AvatarFallback,
  AvatarImage,
...
function OrgSection({
  ...
  // Sprint A: hub PATCH /api/organizations/me only accepts displayName. Slug
  // change isn't exposed via the API yet, so it's read-only here.
  const dirty = name !== initialName;

  const save = useMutation({
    mutationFn: () =>
      api<OrgPatchResponse>("/api/organizations/me", {
        method: "PATCH",
  ...
  return (
    ...
    >
      ...
      <Field
        label="Slug"
        help="Read-only in sprint A — hub does not yet expose slug change. Used in URLs."
      >
        <input
          type="text"
          value={slug}
          disabled
          className="w-full cursor-not-allowed rounded-md border bg-muted/40 px-3 py-2 font-mono text-sm text-muted-foreground"
        />
      </Field>
