...
export function SkillDrawer({
  ...
  const detail = useQuery({
    ...
    // Sprint A: v0 /api/skill → hub /api/skills.
    queryFn: () => api<SkillDetail>(`/api/skills/${skillId}`),
    enabled: open,
  });

  // Sprint A (AE1): hub has no /api/skills/:id/dry-run (M9 P26 defers skill
  // execution engine to V2). The Dry-run button is rendered disabled with a
  // tooltip; the mutation below is kept as a stub for V2 wiring.
  const dryRun = {
    isPending: false,
    data: undefined as { output: string; latencyMs: number } | undefined,
    error: undefined as unknown,
    mutate: () => {
      /* disabled in sprint A */
    },
  };

  const archive = useMutation({
    mutationFn: () =>
      api(`/api/skills/${skillId}`, { method: "DELETE" }),
    onSuccess: () => {
      onChanged();
      onClose();
    },
            ...
            <>
              ...
              <div className="flex flex-wrap gap-2 border-t pt-3">
                <button
                  type="button"
                  disabled
                  title="Skill execution engine ships in V2 (M9 P26)"
                  className="inline-flex items-center gap-1.5 rounded-md border bg-card px-2 py-1 text-xs opacity-50"
                >
