...
export default function InboxPage() {
  ...
  const inbox = useQuery({
    ...
    queryFn: () => {
      ...
      if (typeFilter) params.set("type", typeFilter);
      if (counterpartFilter) params.set("counterpart", counterpartFilter);
      return api<InboxResponse>(`/api/a2a-messages/inbox?${params.toString()}`).then(
        (data) => {
          setPages([data.items]);
  ...
  const otherInbox = useQuery({
    ...
    queryFn: () => api<InboxResponse>(`/api/a2a-messages/inbox?tab=${otherTab}`),
    refetchInterval: 15_000,
  });
  ...
  const employees = useQuery({
    ...
    queryFn: () => api<EmployeeRow[]>("/api/employees?limit=500"),
    staleTime: 60_000,
  });
  ...
  const loadEarlier = useMutation({
    mutationFn: async () => {
      ...
      if (typeFilter) params.set("type", typeFilter);
      if (counterpartFilter) params.set("counterpart", counterpartFilter);
      return api<InboxResponse>(`/api/a2a-messages/inbox?${params.toString()}`);
    },
    onSuccess: (data) => {
      if (!data) return;
  ...
  const action = useMutation({
    mutationFn: async (input: { item: InboxItem; verb: "approve" | "reject" }) => {
      ...
      if (item.kind === "task_review") {
        endpoint = `/api/tasks/${item.id}/review`;
        body = {
          decision: verb === "approve" ? "approved" : "rejected",
        };
      } else if (tab === "approve") {
        endpoint = `/api/a2a-messages/${item.id}/${verb}`;
      } else {
        endpoint =
          verb === "approve"
            ? `/api/a2a-messages/${item.id}/accept`
            : `/api/a2a-messages/${item.id}/reject-receive`;
      }
